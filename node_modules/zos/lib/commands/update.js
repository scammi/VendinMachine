...
const register = (program) => program
    ...
    .withNetworkOptions()
    .action(action);
function action(contractFullNameOrAddress, options) {
    return __awaiter(this, void 0, void 0, function* () {
        const { network, txParams } = yield ConfigVariablesInitializer_1.default.initNetworkConfiguration(options);
        if (!(yield prompt_migration_1.hasToMigrateProject(network)))
            process.exit(0);
        const { initMethod, initArgs } = input_1.parseInit(options, 'initialize');
        ...
        const args = lodash_pickby_1.default({ contractAlias, packageName, proxyAddress, initMethod, initArgs, all, force });
