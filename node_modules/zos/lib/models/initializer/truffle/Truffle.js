...
const Truffle = {
    existsTruffleConfig(path = process.cwd()) {
    ...
    validateAndLoadNetworkConfig(network, force = false, path = process.cwd()) {
        const config = this.getConfig(force);
        const { networks: networkList } = config;
        if (!networkList[network])
            throw Error(`Given network '${network}' is not defined in your ${this._getTruffleConfigFileName(path)} file`);
    ...
    _getTruffleConfigFileName(path) {
        const truffleFile = `${path}/truffle.js`;
